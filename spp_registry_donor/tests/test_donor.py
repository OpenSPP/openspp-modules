...
class TestDonorRegistry(TransactionCase):
    ...
    def test_03_donor_manager_access(self):
        ...
        self.assertEqual(new_donor.name, "Updated Donor Name", 
                        "Manager should be able to update donors")
        
        # Should be able to delete donor
        new_donor.unlink()
        self.assertFalse(manager_env.search([("id", "=", new_donor.id)]),
                        "Manager should be able to delete donors")

    def test_04_action_window(self):
        """Test the donor action window"""
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "spp_registry_donor.action_registry_donor"
        )
        
        # Check action configuration
        self.assertEqual(action["res_model"], "res.partner", 
                        "Action should target res.partner model")
        self.assertEqual(action["domain"], [("is_donor", "=", True)],
                        "Action should filter donors only")
        self.assertIn("default_is_donor", action["context"],
                     "Action should set default donor context")
