...
class OpensppIndicatorPrefetchWizard(models.TransientModel):
    ...
    enqueue = fields.Boolean(default=True)
    chunk_size = fields.Integer(default=2000)
    ...
    def action_run(self):
        ...
        if self.enqueue:
            ...
            return self._notify("Enqueued", f"Created {jobs} refresh job(s) for {len(subject_ids)} subjects.")
        else:
            svc.evaluate(self.metric, model_name, subject_ids, self.period_key, mode="refresh")
            return self._notify("Refreshed", f"Refreshed {len(subject_ids)} subjects now.")
    ...
    def _build_push_summary(self, metric_name: str) -> str:
        ...
        if last_value:
            fetched = last_value.fetched_at or last_value.updated_at
            if fetched:
                parts.append(f"Last push fetched_at: {fields.Datetime.to_string(fetched)}")
        return "\n".join(parts)
