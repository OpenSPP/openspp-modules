...
class EventDataAgriProdSalesCostTechDuringColdDrySeasonTechApplied(models.Model):
    _name = "spp.event.agri.prod.sales.cost.tech.ds.tech"
    _description = (
        "Agricultural Production, Sales, Cost and Technologies During Cold DS (Technologies/Techniques Applied)"
    )

    agri_prod_sales_cost_tech_ids = fields.One2many(
        "spp.event.agri.prod.sales.cost.tech.ds.tech.lines",
        "agri_prod_sales_cost_tech_id",
        string="Agricultural Production, Sales, Cost and Technologies During Cold DS (Technologies/Techniques Applied)",
    )


class EventDataAgriProdSalesCostTechDuringColdDrySeasonTechAppliedLines(models.Model):
    _name = "spp.event.agri.prod.sales.cost.tech.ds.tech.lines"
    _description = (
        "Agricultural Production, Sales, Cost and Technologies During Cold DS Lines (Technologies/Techniques Applied)"
    )

    agri_prod_sales_cost_tech_id = fields.Many2one(
        "spp.event.agri.prod.sales.cost.tech.ds.tech",
        string="Agricultural Production, Sales, Cost and Technologies during Cold DS (Technologies/Techniques Applied)",
    )
