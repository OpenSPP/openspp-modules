...
MODEL_NAME = "spp.event.inc.non.agri"


class TestEventDataIncomeFromNonAgriculture(TransactionCase):

    """
    Test for `spp.event.inc.non.agri` model.
    """
    ...
    def test_04_active_mock_event_data(self):
        mock_event_1 = self.create_mock_event_data()
        mock_event_2 = self.create_mock_event_data()
        self._test_partner._compute_active_event_inc_non_agri()
        self.assertEqual(
            self._test_partner.active_event_inc_non_agri.id,
            mock_event_2.res_id,
