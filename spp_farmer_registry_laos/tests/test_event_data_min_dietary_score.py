...
MODEL_NAME = "spp.event.min.dietary.score"


class TestEventDataMinDietaryDiversifyScore(TransactionCase):

    """
    Test for `spp.event.min.dietary.score` model.
    """
    ...
    def test_04_active_mock_event_data(self):
        mock_event_1 = self.create_mock_event_data()
        mock_event_2 = self.create_mock_event_data()
        self._test_partner._compute_active_event_min_dietary_score()
        self.assertEqual(
            self._test_partner.active_event_min_dietary_score.id,
            mock_event_2.res_id,
