from odoo.tests import TransactionCase


class TestIRModuleModule(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.IrModule = cls.env["ir.module.module"]

        cls.survey_module = cls.IrModule.search([("name", "=", "spreadsheet_dashboard")], limit=1)
        cls.survey_module.button_install()

    def test_01_update_menu_icons(self):
        # Verify that the icon was updated
        self.survey_module.update_menu_icons()
        menu = self.env.ref("spreadsheet_dashboard.spreadsheet_dashboard_menu_root")
        self.assertEqual(menu.web_icon, "spp_registry_base,static/description/icon-Dashboards-White-line.png")
