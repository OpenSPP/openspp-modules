...
class EventTypeDefinition(models.Model):
    ...
    technical_name = fields.Char(
        ...
        required=True,
        help="Model name (e.g., spp.event.compliance.attendance)"
    )
    ...
    field_definitions = fields.Text(
        string="Field Definitions",
        help="JSON array of field definitions"
    )
    ...
    model_deployed = fields.Boolean(
        ...
        default=False,
        help="Indicates if the model has been created"
    )
    view_deployed = fields.Boolean(
        string="Views Deployed",
        default=False,
        help="Indicates if views have been created"
    )
    wizard_deployed = fields.Boolean(
        string="Wizard Deployed",
        default=False,
        help="Indicates if wizard has been created"
    )
    ...
    def _deploy_views(self):
        ...
        existing_tree = self.env["ir.ui.view"].search([
            ("name", "=", tree_view_vals["name"])
        ], limit=1)
        ...
        existing_form = self.env["ir.ui.view"].search([
            ("name", "=", form_view_vals["name"])
        ], limit=1)
...
<form string="{self.name}">
    ...
    def _register_event_type(self):
        ...
        _logger.info("Event type %s registered (selection update requires restart)", 
                    self.technical_name)
