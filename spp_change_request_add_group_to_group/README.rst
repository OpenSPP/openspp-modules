...
The `spp_change_request_add_group_to_group <spp_change_request_add_group_to_group>`__
...
-  **Custom Change Request Model**: Introduces the
   ``spp.change.request.add.group.to.group`` model, inheriting from the base
...
-  **Tailored Forms**: Provides specialized views for creating,
   displaying, and validating ``Add Group to a Group`` change requests, including
   a dedicated form (``view_change_request_add_group_to_group_form``) with
