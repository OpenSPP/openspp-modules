...
class TestSppServicePointDevice(Common):
    ...
    def test_02_action_change_is_active(self):
        ...
        self.assertTrue(device.status, "Status for device should be True as default")
        device.action_change_is_active()
        self.assertFalse(device.status, "Status for device should be changed")
        device.action_change_is_active()
        self.assertTrue(device.status, "Status for device should be changed")
